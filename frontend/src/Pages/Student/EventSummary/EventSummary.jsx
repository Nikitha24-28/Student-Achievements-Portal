...
const EventSummary = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Get student details
        const studentRes = await fetch(`http://localhost:5000/student-details/${email}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        const notifRes = await fetch(`http://localhost:5000/summary-notifications/${formData.s_reg_no}`);
        const notifData = await notifRes.json();
