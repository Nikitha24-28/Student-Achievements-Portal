...
const Events = () => {
  ...
  const fetchApprovedEvents = () => {
    setLoading(true);
    axios
      .get("http://localhost:5000/fetch-approved-events")
      .then((response) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/get-registered-events/${email}`)
      .then((res) => {
  ...
  const handleAddEvent = () => {
    ...
    axios
      .post("http://localhost:5000/submit-for-approval", eventData)
      .then(() => {
  ...
  const handleRegisterClick = (event) => {
    const email = localStorage.getItem("email");
    axios
      .get(`http://localhost:5000/get-registration-info/${email}`)
      .then((res) => {
  ...
  const handleConfirmRegister = () => {
    ...
    if (selectedEvent.event_type === "Team") {
      ...
      apiUrl = "http://localhost:5000/register-team-event";
    } else {
      regData = {
      ...
      apiUrl = "http://localhost:5000/register-event";
    }
    ...
    const checkBlock = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:5000/api/registration-block-status/${studentDetails.reg_no}`
        );
